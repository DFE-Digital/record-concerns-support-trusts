...
describe("User can add case actions to an existing case", () => {
    ...
    it("Should create a NTI warning letter case action", () => {
        ...
        ntiAddPage.getNtiTableRow().should(($row) => {
            ...
            expect($row.eq(0).text().trim()).to.contain(today).and.to.match(/Date NTI: Warning letter opened/i);
            expect($row.eq(1).text().trim()).to.contain("Sent to trust").and.to.match(/Current status/i);
            expect($row.eq(2).text().trim()).to.contain("20-06-2023").and.to.match(/(Date sent)/i);
            expect($row.eq(3).text().trim()).to.contain("Cash flow problems").and.to.match(/(Reasons)/i);
            expect($row.eq(4).text().trim()).to.contain("Action plan").and.to.match(/(Conditions)/i);
