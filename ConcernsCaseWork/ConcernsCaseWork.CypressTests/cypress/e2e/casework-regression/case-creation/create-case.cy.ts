...
import { SourceOfConcernExternal, SourceOfConcernInternal } from 'cypress/constants/selectorConstants';
...
describe('Creating a case', () => {
    ...
    it('Should validate adding a case', () => {
        ...
        createCasePage.createCase().withTrustName('Ashton West End Primary Academy').selectOption();
    ...
    it('Should display an error if no trust is selected', () => {
        createCasePage.createCase().withTrustName('A').confirmOption().hasValidationError('A trust must be selected');
    });
    ...
    describe('When we create a case with the minimum data', () => {
        ...
        beforeEach(() => {
            ...
            cy.createNonConcernsCase(request).then((response) => {
                expectedCaseId = response.urn;
            });
        });

        it('Should render on case management', () => {
            caseManagementPage.getCaseIDText().then((actualCaseId) => {
