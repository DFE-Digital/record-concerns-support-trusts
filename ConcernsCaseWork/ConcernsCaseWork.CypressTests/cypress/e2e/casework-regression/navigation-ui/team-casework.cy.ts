...
describe('Team casework tests', () => {
    describe('When we create a case for a team member', () => {
        ...
        it('Should be able to add and remove team member', () => {
            ...
            teamCaseworkPage.selectTeamMember(name).hasTeamMember(email, name).save();
            ...
            teamCaseworkPage.removeAddedColleagues().hasNoTeamMember(name).save().hasNoCases();
            ...
            teamCaseworkPage.enterTeamMember(name).hasNoResultsFound().save();

            Logger.log(
                'Ensure that when we go back to the home page and back to select colleagues we cannot select previously selected members'
            );
            homePage.selectColleagues();
        ...
        describe('When we have many open cases', () => {
            ...
            beforeEach(() => {
                ...
                caseApi.getTeamByTeam(ownerid).then((response) => {
                ...
                caseApi.getOpenCasesForTeamByOwner(Cypress.env(EnvUsername)).then((response) => {
