...
describe('Trust overview ', () => {
    ...
    describe('When we view a trust on a case', () => {
        ...
        describe('Concerns case', () => {
            beforeEach(() => {
                cy.basicCreateCase().then((response: CreateCaseResponse) => {
                    caseId = response.urn + '';
            ...
            it('Should display trust details on case management and the cases that belong to it', () => {
                ...
                caseMangementPage.viewTrustOverview();
        ...
        describe('Non concerns case', () => {
            beforeEach(() => {
                cy.createNonConcernsCase().then((response: CreateCaseResponse) => {
                    caseId = response.urn + '';
                    trustUkprn = response.trustUkprn;
                });
            });
            ...
            it.skip('Should display the content correctly for a non concerns case with case actions', () => {
                ...
                caseworkTable.getRowByCaseId(caseId).then((row) => {
                    row.hasAction('Action: TFF (trust financial forecast)');
                });

                Logger.log('Check on a closed non concerns case');
                caseMangementPage.viewCase();

                actionSummaryTable.getOpenAction('TFF (trust financial forecast)').then((row) => {
                    row.select();
                });
