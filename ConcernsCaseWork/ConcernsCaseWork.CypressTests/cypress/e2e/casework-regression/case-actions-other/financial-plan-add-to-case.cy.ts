...
describe('User can add Financial Plan case action to an existing case', () => {
    ...
    it('Should add a financial plan', () => {
        ...
        viewFinancialPlanPage.hasDateOpened(toDisplayDate(now)).hasPlanRequestedDate('06 July 2022').hasNotes('Notes!');
    ...
    it('Should close a financial plan', () => {
        ...
        actionSummaryTable.getClosedAction('Financial Plan').then((row) => {
            row.hasName('Financial Plan');
    ...
    it('Should only let one financial plan be created per case', () => {
        ...
        AddToCasePage.hasValidationError(
            'There is already an open Financial Plan action linked to this case. Please resolve that before opening another one.'
        );
    ...
    it('Should add and delete a financial plan', () => {
        ...
        viewFinancialPlanPage.delete();

        deleteFinancialPlanPage.delete();

        Logger.log('Confirm Financial Plan no longer exist');
        actionSummaryTable.assertRowDoesNotExist('Financial Plan', 'open');
    });
    ...
    function checkFormValidation() {
        ...
        editFinancialPlanPage.hasNotes('Notes for validation');
