...
describe('User can add tte to an existing case', () => {
    ...
    it('Creating, editing, validating then viewing a targeted trust engagement', function () {
        ...
        editTargetedTrustEngagementPage
            ...
            .save()
            .hasValidationError(DateInvalidError.replace('{0}', 'Date engagement start'));
        ...
        editTargetedTrustEngagementPage
            ...
            .save()
            .hasValidationError(DateIncompleteError.replace('{0}', 'Date engagement start'))
        ...
        actionSummaryTable.getOpenAction('TTE - Budget Forecast Return/Accounts Return driven').then((row) => {
    ...
    it('Closing TTE', function () {
        ...
        Logger.log('Selecting the TTE from open cases and validating it before closing it');
        ...
        Logger.log('Selecting TTE from closed cases and verifying that the details matches the above');

        Logger.log('Selecting TTE from closed actions');
        actionSummaryTable.getClosedAction('TTE - Budget Forecast Return/Accounts Return driven').then((row) => {
            row.hasName('TTE - Budget Forecast Return/Accounts Return driven');
    ...
    it('Creating and deleting a TTE', function () {
        ...
        viewTargetedTrustEngagementPage.deleteTTE();

        deleteTargetedTrustEngagementPage.delete();

        Logger.log('Confirm TTE no longer exist');
        actionSummaryTable.assertRowDoesNotExist('TTE - Budget Forecast Return/Accounts Return driven', 'open');
    });
});
