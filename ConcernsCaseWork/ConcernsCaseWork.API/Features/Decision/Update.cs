...
namespace ConcernsCaseWork.API.Features.Decision
{
	...
	public class Update
	{
		...
		public class Command : IRequest<int>
		{
			...
			public Command(long DecisionId, DecisionModel model)
			{
		...
		public class CommandHandler : IRequestHandler<Command, Int32>
		{
			...
			public async Task<int> Handle(Command request, CancellationToken cancellationToken)
			{
				...
				var t = request.DecisionId;


				var cr = await _context.Decisions.SingleOrDefaultAsync(f => f.DecisionId == request.DecisionId);
